...
# Задача 2. В фермерском хозяйстве в Карелии выращивают чернику. Черника растет на круглой грядке,
...
# с одного куста и с двух соседних кустов. Собирающий модуль находится перед определенным кустом,
...
max = 0
sum = 0
while mid_index < len(arr):
    left_index = mid_index - 1
    right_index = mid_index + 1
    if mid_index == 0:
        sum = arr[-1] + arr[mid_index] + arr[right_index]
    elif mid_index == len(arr) - 1:
        sum = arr[left_index] + arr[mid_index] + arr[0]
    else:
        sum = arr[left_index] + arr[mid_index] + arr[right_index]
    if sum > max:
        max = sum
    mid_index += 1
print(max)
